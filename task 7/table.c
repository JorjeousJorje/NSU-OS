...
bool fill_table(dynamic_array* table, int fd)
{
    ...
    if (fstat(fd, &stat_buf) < 0) { // define file size
        perror("fstat() error: ");
        return false;
    }

    if ((buffer = mmap(0, stat_buf.st_size, PROT_READ, MAP_SHARED, fd, 0)) == MAP_FAILED) {
        perror("mmap() error: ");
        return false;
    }
    char* n_pos = buffer;
    size_t index = 0;
    for(index = 0; index < stat_buf.st_size; ++index) {
	    if(buffer[index] == '\n') {
	        if (!array_insert(table, index)) {
            	return false;
	        }
        }
    }
    // dont know how to push null terminator to use strchr()
    /*
    while ((n_pos = strchr(n_pos, '\n')) != NULL) {
        const int end_line_pos = n_pos - buffer;
        if (!array_insert(table, end_line_pos)) {
            return false;
        }
        ++n_pos;
    }*/
...
int is_instream_empty(const long interval, const int fd, fd_set* set, struct timeval* tv)
{
    ...
    int number_of_fd = select(1, set, NULL, NULL, tv);
